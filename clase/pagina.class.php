...
class pagina
{
	...
	function pagina($configuracion)
	{
		...
		$miobjeto=new autenticacion($this->id_pagina,$configuracion);
			...
			else
			{
				...
				if(!isset($_REQUEST["no_pagina"]))
				{
					...
					if(isset($_REQUEST["xajax"]))
					{
						...
						$i=0;
						include_once($configuracion["raiz_documento"].$configuracion["clases"]."/funciones_ajax.class.php");
						while(isset($funciones_ajax[$i]))
						{
							$GLOBALS["xajax"]->registerExternalFunction($funciones_ajax[$i],$configuracion["host"].$configuracion["site"].$configuracion["clases"]."funciones_ajax.class.php",XAJAX_POST);
							$i++;
						}
					...
					if(isset($_REQUEST["googlemaps"]))				//class="centralcuerpo"
					{
